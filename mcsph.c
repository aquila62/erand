...
int main(void)
   {
   int i;                     /* loop counter                 */
   int erandelap;            /* elapsed time for erand        */
   ...
   clock_t erandstart;       /* start time for erand          */
   clock_t erandfin;         /* end   time for erand          */
   ...
   erandstart = times(&t);    /* start time for erand      */
   while (i--)                /* loop 1 million times      */
      ...
      x = erandunif(er);     /* uniform number 0-1            */
      y = erandunif(er);     /* uniform number 0-1            */
      z = erandunif(er);     /* uniform number 0-1            */
   ...
   erandfin = times(&t);      /* finish time for erand  */
   printf("Monte Carlo Calculation of Pi/6\n");
   printf("      n = 1 million\n");
   printf("      erand Pi/6 %18.15f\n", ratio);
   /************************************************************/
   ...
   erandelap = erandfin - erandstart;
   mtelap    = mtfin    - mtstart;
   ranlxelap = ranlxfin - ranlxstart;
   printf("      erand ticks %6d\n", erandelap);
