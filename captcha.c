...
int main()
   ...
   /* the erand random number generator uses the IBM */
   /* rand() algorithm plus the Bays-Durham shuffle */
   erfmt *er;               /* erand structure */
   er = (erfmt *) erandinit();    /* initialize the erand generator */
   /*******************************************/
   ...
   *p = '\0';        /* terminate the text string */
   ...
   picrowsz = 3 * ((txtlen*15) + 80 + (erandint(er,40)));
   piccolsz = 24 + 40 + (erandint(er,40));
   ...
   while (txtindx < txtlen)
	 ...
	 while (col < 12)
	    ...
	    if (*fntp == '*')
	       ...
	       *picp++ = erandpwr(er,6);     /* 0-63 color depth */
	       *picp++ = erandpwr(er,6);     /* 0-63 color depth */
	       *picp++ = erandpwr(er,6);     /* 0-63 color depth */
	       } /* if asterisk in font */
	    col++;
