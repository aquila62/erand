/* erand.c - random number generator Version 1.0.0                   */
...
/* 32-bit integer using the RAND algorithm plus a       */
/* Bays-Durham shuffle.  The previous two cycles are    */
/* xor'd together with the current cycle.               */
/* The speed of erand is 1/3 faster than the GSL        */
/* subroutine gsl_rng_get(r).                           */
/********************************************************/

#include "erand.h"

unsigned int erand(erfmt *er)
   {
   ...
   er->prev  = er->x;      /* prev <== current               */
   /* calculate new current state                            */
   /* using the RAND algorithm                               */
   /* The  macro is in erand.h                               */
   ...
   *p    = er->x;
   er->x = tmp;
   /*********************************************************/
   /* return the output of the erand cycle after the swap   */
   /*********************************************************/
   er->out = (er->x ^ er->prev ^ er->pprev);
   return(er->out);
   } /* erand subroutine */
